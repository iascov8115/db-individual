...
-- foreign tables, указывающие на удалённые orders
CREATE FOREIGN TABLE orders_romania (
    order_id               INTEGER,
    customer_id            INT,
...
CREATE FOREIGN TABLE orders_bulgaria (
    order_id               INTEGER,
    customer_id            INT,
    order_date             TIMESTAMP,
    required_delivery_date DATE,
...
SELECT * FROM orders            -- локальные (например, Moldova / остальные страны)
UNION ALL
SELECT * FROM orders_romania
UNION ALL
SELECT * FROM orders_bulgaria;
...
CREATE TRIGGER trg_route_orders
    INSTEAD OF INSERT ON orders_distributed
    FOR EACH ROW
